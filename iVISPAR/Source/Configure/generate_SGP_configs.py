...
from find_shortest_move_sequence import a_star, calculate_manhattan_heuristic, find_config_by_random_expand
...
def generate_SGP_configs(config_id,board_size, num_geoms_min_max, complexity_min_max, complexity_bin_size, shapes,
                         colors, interval = 60):
    """
    ...
    # Set up directories
    config_id = config_id if config_id else f"SGP_ID_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
    ...
    for i, num_geoms in enumerate(range(num_geoms_min_max['min'], num_geoms_min_max['max']+1)):
        ...
        for path_length in range(complexity_min_max['c1']['min'], complexity_min_max['c1']['max'] + 1):
            ...
            while True:
                ...
                #TODO this now only works for c2==0
                if complexity_bins.loc[complexity["c1"], complexity["c2"]] >= complexity_bin_size:
...
if __name__ == "__main__":
    ...
    # Your repo already contains `Data/Params/params_SGP_config.json` (300-sample setup),
    # but older code referenced `params_SGP_config_ICML.json` which may not exist locally.
    base_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..'))
    params_dir = os.path.join(base_dir, 'Data', 'Params')
    candidate_param_files = ['params_SGP_config_ICML.json', 'params_SGP_config.json']
    chosen_param_file = None
    for f in candidate_param_files:
        if os.path.exists(os.path.join(params_dir, f)):
            chosen_param_file = f
            break
    if chosen_param_file is None:
        raise FileNotFoundError(
            f"Could not find any SGP params file in {params_dir}. "
            f"Tried: {', '.join(candidate_param_files)}"
        )

    params = util.load_params_from_json(chosen_param_file)

    # Generate Sliding Geom Puzzle (SGP) configuration files
    config_id = generate_SGP_configs(config_id=params.get('config_id', None),
                                     board_size=params.get('board_size', 4),
                                     num_geoms_min_max=params.get('num_geoms_min_max', {"min": 6, "max": 6}),
                                     complexity_min_max=params.get('complexity_min_max', {"c1": {"min": 12, "max": 12},
                                                                                          "c2": {"min": 0, "max": 0}}),
                                     complexity_bin_size= params.get('complexity_bin_size', 100),
                                     shapes=params.get('shapes', ['cube', 'sphere', 'cylinder', 'pyramid']),
                                     colors=params.get('colors', ['red', 'green', 'blue', 'yellow']))
