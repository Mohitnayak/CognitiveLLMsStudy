...
class GameSystem:
    ...
    def __init__(self, experiment_path, instruction_prompt_file_path, chain_of_thoughts, representation_type, predict_board_state):
        ...
        # Latest parsed Unity payload (dict) for online RL / diagnostics; updated in feed_sim_response.
        self.last_sim_dict = None
    ...
    def feed_sim_response(self, response, i): #TODO make this i step number internal param of game
        ...
        try:
            ...
            if isinstance(sim_message, dict):
                ...
                self.last_sim_dict = dict(sim_message)
            else:
        ...
        # Add modality data saved to disk.
        # Important: for `text` mode we must NOT try to decode `payload` as an RGBA image.
        if self.representation_type == 'vision':
        ...
        elif self.representation_type == 'text':
            # In text-only mode Unity observations still include `board_state`,
            # but may not include a valid screenshot payload.
            filename = os.path.join(self.obs_dir, f"obs_{i}_text")
            image = None
        ...
        # Save the image as a PNG file (only for non-text modes)
        if self.representation_type != 'text':
            try:
