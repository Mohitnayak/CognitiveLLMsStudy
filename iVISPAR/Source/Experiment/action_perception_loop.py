...
async def initialize_connection(uri):
    ...
    """
    # Screenshot payloads can be large; raise max_size to avoid websocket code 1009.
    websocket = await websockets.connect(uri, max_size=50000000, ping_interval=10, ping_timeout=360)
    try:
...
async def interact_with_server(websocket, network_id, partner_id, agent, game, episode_logger):
    ...
    while not game.check_done(message_data):
        ...
        if message_data.get("command") == "Screenshot" or message_data.get("command") == "ActionAck":
            ...
            if getattr(agent, "act_needs_game_reference", False):
                user_message = agent.act(observation, i, game)
            else:
    ...
    # Persist terminal Unity payload (loop may exit before another act() call)
    observation = game.feed_sim_response(message_data, i)
    finalize = getattr(agent, "finalize_episode_rl", None)
    if callable(finalize):
        try:
            finalize(message_data, game)
        except Exception as e:
            logging.error("finalize_episode_rl failed: %s", e)
    game._save_logs()
