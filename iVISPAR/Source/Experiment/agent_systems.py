...
import os
import logging
# Anthropic + Gemini SDKs are optional (local Ollama runs use `agent.OllamaAgent` at repo root).
try:
    ...
    logging.warning(f"Anthropic SDK not available; ClaudeAgent will fail to initialize. Reason: {e}")
...
try:
    ...
    logging.warning(f"Gemini SDK not available; GeminiAgent will fail to initialize. Reason: {e}")
from abc import ABC, abstractmethod
import re
from PIL import Image, ImageDraw, ImageFont
import experiment_utilities as util

# Open-source (torch/vLLM) deps are optional.
# We guard these imports so Ollama-only runs don't crash if `torch` isn't installed.
try:
    from qwen_vl_utils import process_vision_info
    from transformers import AutoProcessor, AutoTokenizer
    from vllm import LLM, SamplingParams
    OPEN_SOURCE_DEPS_AVAILABLE = True
except (ModuleNotFoundError, ImportError) as e:
    ...
    LLM = None
    SamplingParams = None
    OPEN_SOURCE_DEPS_AVAILABLE = False
    logging.warning(
        "Optional vision agent deps not available; open-source agents will fail to initialize. "
        f"Reason: {e}"
    )
...
def load_api_keys(api_key_file_path):
    ...
    Returns an empty dict when `api_key_file_path` is not provided (useful for
    local/self-hosted agents like Ollama).
    """
    ...
    if not os.path.exists(api_key_file_path):
        raise FileNotFoundError(f"API keys file not found: {api_key_file_path}")
...
class ClaudeAgent(LLMAgent):
    ...
    def __init__(self, episode_path, episode_logger, api_key_file_path, instruction_prompt_file_path, visual_state_embedding, single_images=True, COT=False, delay=0, max_history=0):
        if not ANTHROPIC_AVAILABLE or Anthropic is None:
            raise ImportError(
                "ClaudeAgent requires the optional `anthropic` SDK. "
                "Install it or use an OllamaAgent instead."
            )
        super().__init__(episode_path, episode_logger, api_key_file_path, instruction_prompt_file_path, visual_state_embedding, single_images, COT, delay, max_history)
...
class GeminiAgent(LLMAgent):
    def __init__(self, episode_path, episode_logger, api_key_file_path, instruction_prompt_file_path, visual_state_embedding, single_images=True, COT=False, delay=0, max_history=0):
        if not GENAI_AVAILABLE or genai is None:
            raise ImportError(
                "GeminiAgent requires the optional `google-generativeai` SDK. "
                "Install it or use an OllamaAgent instead."
            )
        super().__init__(episode_path, episode_logger, api_key_file_path, instruction_prompt_file_path, visual_state_embedding, single_images, COT, delay, max_history)
...
class OpenSourceAgent:
    def __init__(self, model_path, instruction_prompt_file_path, max_history, visual_state_embedding, n_gpus, COT=False):
        if not OPEN_SOURCE_DEPS_AVAILABLE:
            raise ImportError(
                "Open-source vision agents require torch/transformers/vLLM. "
                "Install `torch` and related deps or use an OllamaAgent instead."
            )
        self.n_gpus = n_gpus
