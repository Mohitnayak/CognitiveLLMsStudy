...
async def start_server_fix(BUILD_DIRECTORY):
    ...
    # Use the build directory passed in (run_experiment.py sets this to the repo's `iVISPAR/` build).

    # Change working directory to the WebGL build folder
    if not os.path.isdir(BUILD_DIRECTORY):
        raise FileNotFoundError(
            f"WebGL build directory not found: {BUILD_DIRECTORY}. "
            f"Set it via run_socketserver_in_background(...) / webApp_dir."
        )
