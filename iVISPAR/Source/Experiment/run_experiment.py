...
async def run_experiment(games, agents, envs, experiment_id=None):
    ...
    # Log the exact parameters used for this run (helps confirm you aren't using an old config).
    try:
        experiment_logger.info("Agents (from params JSON):\n%s", json.dumps(agents, indent=2))
        experiment_logger.info("Games (from params JSON):\n%s", json.dumps(games, indent=2))
        experiment_logger.info("Envs (from params JSON):\n%s", json.dumps(envs, indent=2))
    except TypeError as e:
        # If any unexpected non-serializable objects slip in, keep the run going.
        experiment_logger.warning("Could not JSON-serialize params summary: %s", e)
        experiment_logger.warning("Raw agents=%s", agents)
        experiment_logger.warning("Raw games=%s", games)
        experiment_logger.warning("Raw envs=%s", envs)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run iVISPAR experiment. Params JSON lives under iVISPAR/Data/Params/."
    )
    parser.add_argument(
        "--params",
        default="params_experiment_OllamaVision.json",
        help="Params filename in Data/Params (default: Ollama-only baseline). "
        "Examples: params_experiment_CognitiveAgent.json (LLM critique rerank), "
        "params_experiment_CognitiveAgentDQN.json (Bellman DQN over K proposals), "
        "params_experiment_OnlineRLAgent.json (DQN on primitive moves).",
    )
    args = parser.parse_args()

    params = util.load_params_from_json(args.params)
    if not params:
        raise SystemExit(
            f"Failed to load params from Data/Params/{args.params!r} (empty dict or missing file)."
        )
